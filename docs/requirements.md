...
| FR-5 | The service shall support proposing complete campaign creation across Campaign, Ad Set, Creative, and Ad, plus edit/manage, activate, pause, resume, and budget-affecting mutations. Creation accepts only the three campaign kinds defined below and derives their fixed Meta combinations internally. Where Meta supports it, validation uses validate-only preflight before persistent creation; preflight never replaces owner approval. Successful creation returns Campaign, Ad Set, Creative, and Ad exactly once; Campaign, Ad Set, and Ad are created `PAUSED`, while Creative is created and bound without a delivery status. Activation requires a separate operation and approval. Delivery transitions apply only to Campaign, Ad Set, and Ad. |
| FR-6 | Creative images and videos shall originate only from host-trusted inbound attachment bytes and metadata supplied by supported public OpenClaw SDK context, never from a model-supplied path or URL. `upload_chat_media` shall fail closed without that trusted context. `original_filename` is basename display metadata of 1..255 characters; slash, backslash, and control characters are rejected and it is never interpreted as a path. `fb-marketing-server` shall validate multipart fields, bytes, content and limits, hash bytes, bind media to scope and operation, and remove staged content after completion or expiry. Deterministic owner commands remain separate. |
...
| AC-14 | Valid image and video bytes from host-trusted inbound attachment context are copied into private staging, content-validated, hashed, and bound to the immutable campaign operation. Missing trusted context and unsupported, oversized, mismatched, expired, model-path, or model-URL inputs fail before a Meta call; deterministic owner commands remain separate, and staged files are removed after completion or expiry. | FR-5, FR-6, SEC-3 |
