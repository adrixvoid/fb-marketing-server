...
| Mutations | Complete Campaign/Ad Set/Creative/Ad creation, edits, activation, pausing, resuming, and budget changes become immutable pending operations. Creation supports exactly three product campaign kinds and requires owner approval. It produces Campaign, Ad Set, and Ad `PAUSED` and creates/binds Creative without delivery status; activation requires separate approval. |
...
| Ingest creative media | Claim exact `/stage-ad-media <client_id> <ad_account_id>` text in trusted `inbound_claim` context before model dispatch. Require OpenClaw's authorized-sender and owner facts plus the exact singleton configured owner before inspecting that event's one fresh local attachment. Reject missing, multiple, pending, expired, unsupported, out-of-root, or symlink inputs; never retain session-to-latest-message state. Call authenticated `/v1/media` with explicit scope, copy bytes to private staging, validate content and Meta limits, hash and scope-bind them, and return only safe staged metadata. PNG validation enforces valid CRCs and the required unique/ordered `IHDR`, optional `PLTE`, contiguous `IDAT`, and terminal `IEND` structure without decoding pixels. |
| Create a complete campaign | Accept one product campaign kind, derive the fixed Meta combination, and validate dynamic assets, media, and local semantics before persistence. Store one immutable operation valid while `now < created_at + 12 hours`. Approval creates Campaign, Ad Set, Creative, and Ad exactly once; only Campaign, Ad Set, and Ad are `PAUSED`, and Creative is bound. |
...
- Revalidation marks an operation stale only for semantic target mismatch or not-found, including changed authority, generation, payload, derived target, or bound media. Retryable, transient, malformed-upstream, and unavailable-upstream failures leave pending/executing state intact for a later safe retry; approval preserves contract-supported `429`/`Retry-After` or normalized `502` responses.
...
`v26.0` is the MVP baseline as of 2026-08-18. Before an upgrade, inventory affected calls, validate endpoint and permission changes, update canonical fixtures and the OpenAPI contract, run read and mutation-validation checks, then prove one scoped pilot before rollout. Campaign daily/lifetime budgets, local monthly pacing budgets, and Meta spend/account limits remain distinct throughout.
