...
| Multipart | `@fastify/multipart` | Stream trusted chat attachments without buffering whole files in application memory. |
...
| `sharp` or `ffmpeg` | Content sniffing and Meta validate-only cover initial requirements; transformation is not required. |
...
- Keep Meta at `v26.0` until contract fixtures, permission behavior, reads, validate-only calls, and one scoped pilot pass on the replacement version.
